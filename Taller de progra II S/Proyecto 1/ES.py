...
window.title("Maquina Dispensadora") #damos titulo a la ventana
...
def login():  # se define login
    ...
    window2.title("Registrase")  #titulo de la ventana
    window2.geometry("20x30") #dimensiones de la ventana
    window2.configure(bg="blue") # se configura el fondo
    entrada= askstring("Contraseña","Ingrese la Contraseña",show="*") # se crea la variable entrada que servira para verficar la contraseña
    ...
    def validar(entrada1): # se define una funcion para validar los parametros de entradas
        if entrada1 == "1434": # se define la condicion de la contraseña
            ...
            messagebox.showwarning("Peligro!!","ERROR LA CONTRASEÑA ES INCORRECTA")# lanza mensaje de error en contraseña

    btnvali = Button(window2,width=10,height=1,text=" CONFIRMAR  ", command= lambda entrada1=entrada: validar(entrada1)).pack()#boton de validacion de la contraseña a segundo paso
...
def Administrador(): #se define la funcion de administrador
    ...
    btnAP= Button(window3,text= "APAGAR",width=10,height=2,font = fuente3, command= Apagar).place(x=350,y=360) # boton de apagar el modo administrador
    btnRE= Button(window3,text= "CORTE DE CONTROL",width=30,height=2,font = fuente3, command= Reset ).place(x=270,y=280)# boton de reset el modo administrador
    btnVD= Button(window3,text= "REPORTE DE VENTA DETALLADO",width=30,height=2,font = fuente3,command=V_Detallada).place(x=270,y=200)# boton de reporte de ventas resumen a}modo administrador
    btnVR= Button(window3,text= "REPORTE DE VENTA RESUMEN",width=30,height=2,font = fuente3, command= V_Resumen).place(x=270,y=100)# boton de areporte de ventas detallado a}modo administrador
    window3.mainloop()
...
def Tabla1():
    prod=(pd.read_csv("Productos.csv",header = 0)) # caraga tabla de productos
...
entclick = Button(window,text="COMPRAR",bg="green",command=total).place(x=560,y=440) #boton de comprar
entclick = Button(window,text="SALIR",bg="red",command = salir).place(x=670,y=440)  # boton de salir
...
def tarjeta(): # define la funcion de compra por tarjeta
    ...
    Label(pago,text= "PROCESANDO....",bg="#B388FF",width = 19,height=2,font= fuente2).place(x=30,y=0)#label de procesando
    def compuerta():
        Label(pago,text= "SU COMPRA HA SIDO EXITOSA",bg="#B388FF",width= 26,height=2,font="ARIAL 12").place(x=10,y=0)
        time.sleep(3)
        Label(window,text= "RETIRE SU PRODUCTO",bg="#B388FF",width= 19,height=2,font= fuente2).place(x=525,y=40)  # se define el proceso de pago con tarjeta
    d1 = Button(window,text= "PRESIONE PARA ABRIR LA COMPUERTA",bg="gray",command=compuerta).place(x=535,y=270)
    pago.withdraw
################################################################   
#Cargar imagenes
...
bg3=PhotoImage(file="Images/fondo2.png")
######################################################################
...
Prod = Label(window,text="INGRESE EL CÓDIGO PRODUCTO:",bg="gray").place(x=540,y=350)
Mpag = Label(window,text="METODOS DE PAGO:",bg="gray").place(x=575,y=94)
Efec = Label(window,text="EFECTIVO:",bg="gray").place(x=498,y=104)
Tarjeta = Label(window,text="TARJETA:",bg="gray").place(x=498,y=184)
